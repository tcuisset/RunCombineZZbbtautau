...
python3 RunNonResLimitsTTRate.py --ver ul_2016_HIPM_ZbbHtt_v12,ul_2016_ZbbHtt_v12,ul_2017_ZbbHtt_v12,ul_2018_ZbbHtt_v12 \
...
python3 RunNonResLimitsTTRate.py --ver ul_2016_HIPM_ZttHbb_v12,ul_2016_ZttHbb_v12,ul_2017_ZttHbb_v12,ul_2018_ZttHbb_v12 \
...
python3 RunNonResLimitsTTRate.py --ver ul_2016_ZZ_v12,ul_2016_HIPM_ZZ_v12,ul_2017_ZZ_v12,ul_2018_ZZ_v12 \
    --cat cat_ZZ_elliptical_cut_90_resolved_1b,cat_ZZ_elliptical_cut_90_resolved_2b,cat_ZZ_elliptical_cut_90_boosted_noPNet \
    --cr_cat cat_ZZ_elliptical_cut_90_CR_resolved_1b,cat_ZZ_elliptical_cut_90_CR_resolved_2b,cat_ZZ_elliptical_cut_90_CR_boosted_noPNet \
    --feat dnn_ZZbbtt_kl_1 --prd prod_240723 --grp datacard_zz --num 5 --TTrate
...
if __name__ == "__main__" :
    ...
    parser.add_argument("--cr_cat",       dest="cr_cat",                default='')
    ...
    if options.TTrate:
        if ',' in options.cat and ',' in options.cr_cat:
            if len(options.cat.split(',')) != len(options.cr_cat.split(',')):
                sys.exit(" ### ERROR: Please define one CR for each SR")
        else:
            if options.cr_cat == '':
                sys.exit(" ### ERROR: Please define one CR")
    ...
    if run_cp:
        for feature in features:
            for version in versions:
                for category in categories + cr_categories:
    ...
    def run_single_limit(feature, version, category, ch, cr_category=''):
        ...
        if options.TTrate:
            if not '2016_ALL' in version:
                fit_options = f'--X-rtd MINIMIZER_no_analytic --setParameterRanges ttbarRate_{GetCatShort(category)}_{GetYear(version)}=0.0,2'
                mask_string = 'r=1,mask_SR=1'
            else:
                fit_options = f'--X-rtd MINIMIZER_no_analytic --setParameterRanges ttbarRate_{GetCatShort(category)}_2016=0.0,2:ttbarRate_{GetCatShort(category)}_2016_HIPM=0.0,2'
                mask_string = 'r=1,mask_SR_Y2016=1,mask_SR_Y2016_HIPM=1'

            datafile_sr = ch_dir + f'/{version}_{category}_{feature}_{grp}_{ch}_os_iso.txt'
            datafile_cr = maindir + f'/{version}/{prd}/{feature}/{cr_category}/{ch}/{version}_{cr_category}_{feature}_{grp}_{ch}_os_iso.txt'
            datafile = ch_dir + f'/{version}_{category}_{feature}_{grp}_{ch}_os_iso_SR_CR.txt'

            cmd = f'combineCards.py SR={datafile_sr} CR={datafile_cr} > {datafile}'
            ...
            cmd = f'cd {ch_dir} &&      text2workspace.py {datafile} --channel-masks -o {ch_dir}/model.root     &>{ch_dir}/text2workspace.log'
            run_cmd(cmd, run)

            print(" ### INFO: Create asimov dataset")
            cmd = f'cd {ch_dir} &&      combine -M GenerateOnly {ch_dir}/model.root -t -1 --setParameters {mask_string} --toysFrequentist --saveToys -n .asimov ' + \
             f'-v3 {fit_options}        &>{ch_dir}/asimov.log'
            run_cmd(cmd, run)

            print(" ### INFO: Run Delta Log Likelihood Scan")
            cmd = f'cd {ch_dir} &&      combine -M MultiDimFit {ch_dir}/model.root -v 3 -t -1 --toysFrequentist --toysFile {ch_dir}/higgsCombine.asimov.GenerateOnly.mH120.123456.root ' + \
             f' --algo=grid --points 100 {r_range} {fit_options}        &>{ch_dir}/multiDimFit.log'
            run_cmd(cmd, run)
            ...
            print(" ### INFO: Run Delta Log Likelihood Scan")
            cmd = f'cd {ch_dir} &&      combine -M MultiDimFit {ch_dir}/model.root --algo=grid --points 100 {r_range} --preFitValue 1 ' + \
             f'--expectSignal 1 -t -1        &>{ch_dir}/multiDimFit.log'
            run_cmd(cmd, run)
    ...
    if run_one:
        ...
        if not options.TTrate:
            ...
            # Run with TTrate floating
            if options.singleThread:
                for feature in features:
                    for version in versions:
                        for category, cr_category in zip(categories, cr_categories):
                            for channel in channels:
                                run_single_limit(feature, version, category, channel, cr_category)
            else:
                with concurrent.futures.ProcessPoolExecutor(max_workers=15) as exc:
                    futures = []
                    for feature in features:
                        for version in versions:
                            for category, cr_category in zip(categories, cr_categories):
                                for channel in channels:
                                    futures.append(exc.submit(run_single_limit, feature, version, category, channel, cr_category))
                    for res in concurrent.futures.as_completed(futures):
                        res.result()
    ...
    def run_comb_channels(feature, version, category):
        ...
        for ch in channels:
            if options.TTrate:
                ch_file = maindir + f'/{version}/{prd}/{feature}/{category}/{ch}/{version}_{category}_{feature}_{grp}_{ch}_os_iso_SR_CR.txt'
            else:
        ...
        if options.TTrate:
            ...
            cmd = f'cd {combdir} &&         text2workspace.py {combdir}/{version}_{feature}_{category}_os_iso.txt --channel-masks -o {combdir}/model.root       &>{combdir}/text2workspace.log'
            run_cmd(cmd, run)
            
            mask_string = 'r=1'
            if not '2016_ALL' in version:
                fit_options = f'--X-rtd MINIMIZER_no_analytic --setParameterRanges ttbarRate_{GetCatShort(category)}_{GetYear(version)}=0.0,2'
                for ch in channels: mask_string += f',mask_{ch}_SR=1'
            else: 
                fit_options = f'--X-rtd MINIMIZER_no_analytic --setParameterRanges ttbarRate_{GetCatShort(category)}_2016=0.0,2:ttbarRate_{GetCatShort(category)}_2016_HIPM=0.0,2'
                for ch in channels: mask_string += f',mask_{ch}_SR_Y2016=1,mask_{ch}_SR_Y2016_HIPM=1'

            cmd = f'cd {combdir} &&         combine -M GenerateOnly {combdir}/model.root -t -1 --setParameters {mask_string} --toysFrequentist --saveToys -n .asimov -v3 {fit_options}      &>{combdir}/asimov.log'
            run_cmd(cmd, run)

            cmd = f'cd {combdir} &&         combine -M MultiDimFit {combdir}/model.root --algo=singles {r_range} -t -1 --toysFrequentist --toysFile {combdir}/higgsCombine.asimov.GenerateOnly.mH120.123456.root {fit_options}      &>{combdir}/multiDimFit_singles.log'
            run_cmd(cmd, run)
            cmd = f'cd {combdir} &&         combine -M MultiDimFit {combdir}/model.root --algo=grid --points 100 {r_range} -t -1 --toysFrequentist --toysFile {combdir}/higgsCombine.asimov.GenerateOnly.mH120.123456.root {fit_options}        &>{combdir}/multiDimFit_grid.log'
            run_cmd(cmd, run)
            ...
            run_cmd(cmd, run)
            cmd = f'cd {combdir} &&         combine -M MultiDimFit higgsCombine.bestfit.with_syst.MultiDimFit.mH125.root {r_range_setPR} ' + \
            ...
            cmd = f'cd {combdir} &&         text2workspace.py {version}_{feature}_{category}_os_iso.txt -o model.root       &>{combdir}/text2workspace.log'
            run_cmd(cmd, run)
            cmd = f'cd {combdir} &&         combine -M MultiDimFit model.root --algo=singles {r_range} --preFitValue 1 --expectSignal 1 -t -1       &>multiDimFit_singles.log'
            run_cmd(cmd, run)
            cmd = f'cd {combdir} &&         combine -M MultiDimFit model.root --algo=grid --points 100 {r_range} --preFitValue 1 --expectSignal 1 -t -1         &>multiDimFit_grid.log'
            run_cmd(cmd, run)
            cmd = f'cd {combdir} &&         combine -M Significance {version}_{feature}_{category}_os_iso.txt -t -1 --expectSignal=1        &> Significance_{version}_{feature}_{category}.log'
            run_cmd(cmd, run)

            cmd = f'cd {combdir} &&         combine -M MultiDimFit model.root -m 125 -n .bestfit.with_syst {r_range_setPR} --saveWorkspace'\
    ...
    def run_comb_categories(feature, version):
        ...
        if options.TTrate:
            ...
            mask_string = 'r=1'
            if not '2016_ALL' in version:
                fit_options = f'--X-rtd MINIMIZER_no_analytic --setParameterRanges ' + ':'.join([f'ttbarRate_{GetCatShort(category)}_{GetYear(version)}=0.0,2' for category in categories])
                for category in categories: 
                    for ch in channels: mask_string += f',mask_{GetCatShort(category)}_{ch}_SR=1'
            else: 
                fit_options = f'--X-rtd MINIMIZER_no_analytic --setParameterRanges ' + \
                 ':'.join([f'ttbarRate_{GetCatShort(category)}_2016=0.0,2:ttbarRate_{GetCatShort(category)}_2016_HIPM=0.0,2' for category in categories])
                for category in categories: 
                    for ch in channels: mask_string += f',mask_{GetCatShort(category)}_{ch}_SR_Y2016=1,mask_{GetCatShort(category)}_{ch}_SR_Y2016_HIPM=1'
            ...
            cmd = f'cd {combdir} &&         combine -M MultiDimFit {combdir}/model.root -m 125 -n .bestfit.with_syst {r_range_setPR} --saveWorkspace' + \
                f' -t -1 --toysFrequentist --toysFile {combdir}/higgsCombine.asimov.GenerateOnly.mH120.123456.root --X-rtd MINIMIZER_no_analytic        &>MultiDimFit.log'
            run_cmd(cmd, run)
            cmd = f'cd {combdir} &&         combine -M MultiDimFit {combdir}/higgsCombine.bestfit.with_syst.MultiDimFit.mH125.root {r_range_setPR} ' + \
                f' --saveWorkspace -n .scan.with_syst.statonly_correct --algo grid --points 100' + \
                f' --snapshotName MultiDimFit --freezeParameters allConstrainedNuisances -t -1 --toysFrequentist --toysFile {combdir}/higgsCombine.asimov.GenerateOnly.mH120.123456.root --X-rtd MINIMIZER_no_analytic      &>MultiDimFit_statOnly.log'
            run_cmd(cmd, run)
            ...
            run_cmd(cmd, run)
            cmd = f'cd {combdir} &&         combine -M MultiDimFit {combdir}/model.root --algo=singles {r_range} --preFitValue 1 --expectSignal 1 -t -1         &>multiDimFit_singles.log'
            run_cmd(cmd, run)
            cmd = f'cd {combdir} &&         combine -M MultiDimFit {combdir}/model.root --algo=grid --points 100 {r_range} --preFitValue 1 --expectSignal 1 -t -1       &>multiDimFit_grid.log'
            run_cmd(cmd, run)
            cmd = f'cd {combdir} &&         combine -M Significance {combdir}/{version}_{feature}_os_iso.txt -t -1 --expectSignal=1         &> Significance_{version}_{feature}.log'
            run_cmd(cmd, run)
    ...
    def run_comb_years(feature):
        ...
        if options.TTrate:
            ...
            cmd = f'cd {combdir} && text2workspace.py {combdir}/FullRun2_{o_name}_{feature}_os_iso.txt --channel-masks -o {combdir}/model.root &>{combdir}/text2workspace.log'
            run_cmd(cmd, run)
            ...
            for version in versions:
                for category in categories:
                    if not '2016_ALL' in version:
                        list_fit_range.append(f'ttbarRate_{GetCatShort(category)}_{GetYear(version)}=0.0,2')
                        for ch in channels:
                            mask_string += f',mask_Y{GetYear(version)}_{GetCatShort(category)}_{ch}_SR=1'
                    else:
                        list_fit_range.append(f'ttbarRate_{GetCatShort(category)}_2016=0.0,2:ttbarRate_{GetCatShort(category)}_2016_HIPM=0.0,2')
                        for ch in channels:
                            mask_string += f',mask_Y{GetYear(version)}_{GetCatShort(category)}_{ch}_SR_Y2016=1,mask_Y{GetYear(version)}_{GetCatShort(category)}_{ch}_SR_Y2016_HIPM=1'

            # pdb.set_trace()

            fit_options = '--X-rtd MINIMIZER_no_analytic --setParameterRanges ' + \
                ':'.join([f'ttbarRate_{GetCatShort(category)}_{GetYear(version)}=0.0,2' for category in categories for version in versions])
            mask_string = ''
            for category in categories: 
                for ch in channels:
                    mask_string += f',mask_Y{GetYear(version)}_{GetCatShort(category)}_{ch}_SR=1'
