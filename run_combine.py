...
if __name__ == "__main__" :
    ...
    parser.add_option("--ch",      dest="ch",       default='etau')
    ...
    # datadir = basedir + f'/ul_2018_ZZ_v10/cat_ZZ_elliptical_cut_80_{ch}/prod_230718_OldDNNScore'
    ...
    odir = os.getcwd() + f'/{ver}/{ch}/{feat}'
    print(" ### INFO: Saving output in ", odir)
    if run: os.system('mkdir -p ' + odir)
    ...
    cmd = f'text2workspace.py {feat}_{ch}_os_iso.txt -o {odir}/model.root'
    ...
    cmd = f'combine -M Significance {feat}_{ch}_os_iso.txt -t -1 --expectSignal=1 --pvalue'
    os.chdir(datadir)
    if run: os.system(cmd)
    if run: os.system(f'mv {datadir}/higgsCombineTest.Significance.mH120.root {odir}/higgsCombineTest.Significance.mH120.pvalue.root')
    ...
    cmd = f'combine -M Significance {feat}_{ch}_os_iso.txt -t -1 --expectSignal=1'
    os.chdir(datadir)
    if run: os.system(cmd)
    if run: os.system(f'mv {datadir}/higgsCombineTest.Significance.mH120.root {odir}/higgsCombineTest.Significance.mH120.significance.root')
